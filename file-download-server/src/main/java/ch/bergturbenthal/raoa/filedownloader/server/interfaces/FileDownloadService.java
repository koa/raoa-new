...
public class FileDownloadService extends FileDownloadServiceGrpc.FileDownloadServiceImplBase {
    ...
    @Override
    public void downloadFile(final FileDownloadRequest request, final StreamObserver<FileFragment> responseObserver) {
        final long handle = request.getDownloadHandle();
        downloadService.downloadFile(handle).subscribe(responseObserver::onNext, responseObserver::onError, responseObserver::onCompleted);
    }
