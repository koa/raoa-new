...
@GRpcService
public class DownloadTestService extends RequestFileDownloadServiceGrpc.RequestFileDownloadServiceImplBase {
    ...
    @Override
    public void requestFileDownload(final FileDownloadRequest request, final StreamObserver<FileDownloadResponse> responseObserver) {
        final long fileHandle = downloadService.anounceFile(() -> new FileInputStream("pom.xml"));
        responseObserver.onNext(FileDownloadResponse.newBuilder().setDownloadHandle(fileHandle).build());
