...
public class SpringSecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  @Autowired private RaoaProperties properties;
  @Autowired private RuntimeConfigurationService runtimeConfigurationService;
  ...
  protected void configure(final HttpSecurity http) throws Exception {
    ...
    final GrantedAuthoritiesMapper userAuthoritiesMapper =
        authorities -> {
  ...
  public void configureGlobal(final AuthenticationManagerBuilder auth) throws Exception {
    final UserDetailsService userDetailsService =
        username -> {
          final Map<String, UserData> knownUsers =
              runtimeConfigurationService.getGlobalConfiguration().getKnownUsers();
  ...
  public AuthenticatedUser currentUser() {
    ...
    if (principal instanceof UserDetails) {
      ...
      return AuthenticatedUser.builder()
          ...
          .email(Optional.of(userDetails.getUsername()))
          .picture(Optional.empty())
          .build();
    }
  ...
  private List<GrantedAuthority> login(final String email) {
    final List<GrantedAuthority> authorities = new ArrayList<>();
    runtimeConfigurationService.editGlobalConfiguration(
        c -> {
          ...
          if (existingUser == null) {
            userDataBuilder =
                UserData.builder()
                    ...
                    .globalAccessLevel(AccessLevel.NONE)
                    .admin(properties.getAdminEmail().equals(email));
          } else {
            userDataBuilder = existingUser.toBuilder();
          }
          final UserData userData =
              userDataBuilder.lastAccess(Instant.now()).localPassword(Optional.empty()).build();
