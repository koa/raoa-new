...
public class EditUsersView extends CustomComponent implements View {
  public EditUsersView(final RuntimeConfigurationService configService) {
    ...
    final Column<Entry<String, UserData>, String> emailColumn =
        grid.addColumn(entry -> entry.getKey());
    emailColumn.setCaption("eMail");

    final AtomicReference<DateTimeFormatter> dateTimeFormatter =
        new AtomicReference<DateTimeFormatter>(
    ...
    addInstantColumn(
        grid,
        "created at",
        (entry) -> entry.getValue().getCreatedAt(),
        dateTimeFormatter,
        currentZone);
    addInstantColumn(
        grid,
        "last login",
        (entry) -> entry.getValue().getLastAccess(),
        dateTimeFormatter,
        currentZone);

    final Column<Entry<String, UserData>, Boolean> adminColumn =
        grid.addColumn(entry -> entry.getValue().isAdmin());
    adminColumn.setCaption("admin");

    adminColumn.setEditorComponent(
        new CheckBox(),
        (bean, fieldvalue) ->
            configService.editGlobalConfiguration(
                GlobalConfigurationData.editUser(
                    bean.getKey(), u -> u.toBuilder().admin(fieldvalue.booleanValue()).build())));
    adminColumn.setEditable(true);

    final Column<Entry<String, UserData>, AccessLevel> accessLevelColumn =
        grid.addColumn(entry -> entry.getValue().getGlobalAccessLevel());
    accessLevelColumn.setCaption("Access");

    final ComboBox<AccessLevel> accessLevelEditor =
        new ComboBox<>("", Arrays.asList(AccessLevel.values()));
    accessLevelEditor.setEmptySelectionAllowed(false);
    accessLevelColumn.setEditorComponent(
        accessLevelEditor,
        (bean, fieldvalue) ->
            configService.editGlobalConfiguration(
                GlobalConfigurationData.editUser(
                    bean.getKey(), u -> u.toBuilder().globalAccessLevel(fieldvalue).build())));
  ...
  private void addInstantColumn(
      final Grid<Entry<String, UserData>> grid,
      final String caption,
      final ValueProvider<Entry<String, UserData>, Instant> valueProvider,
      final AtomicReference<DateTimeFormatter> dateTimeFormatter,
      final AtomicReference<ZoneOffset> currentZone) {
    final Column<Entry<String, UserData>, String> createdAtColumn =
        grid.addColumn(
            entry ->
                dateTimeFormatter
                    .get()
                    .format(valueProvider.apply(entry).atOffset(currentZone.get())));
