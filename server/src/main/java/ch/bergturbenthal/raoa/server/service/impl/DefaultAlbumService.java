...
public class DefaultAlbumService implements AlbumService {
  ...
  private final Map<File, AlbumState> currentAlbumState =
  ...
  public void detectAlbum(final File dir) {
    try {
      log.info("Open " + dir);
      @Cleanup final Git git = Git.open(dir);
      ...
      @Cleanup final RevWalk walk = new RevWalk(repository);
      @Cleanup final TreeWalk treeWalk = new TreeWalk(repository);
      ...
      while (treeWalk.next()) {
        ...
        if (!existingThumbnails.contains(objectId)) {
          ...
          final Resource gitContentResource =
              new GitObjectResource(currentObjectLoader, pathString, objectId);
          thumbnailer
              .createThumbnail(gitContentResource)
              .subscribe(
                  (final Resource result) -> {
                    final ObjectInserter newInserter = repository.getObjectDatabase().newInserter();
                    try {
                      final ObjectId objectId2 =
                          newInserter.insert(
                              Constants.OBJ_BLOB, result.contentLength(), result.getInputStream());
