...
public class DefaultDownloadClient implements DownloadClient {
  ...
  public Flux<ByteString> downloadFile(final long handle, final Channel channel) {
    return Flux.create(
        fluxSink -> {
          final FileDownloadServiceStub downloadServiceStub =
              FileDownloadServiceGrpc.newStub(channel);
          downloadServiceStub.downloadFile(
              FileDownloadRequest.newBuilder().setDownloadHandle(handle).build(),
              new StreamObserver<FileFragment>() {
