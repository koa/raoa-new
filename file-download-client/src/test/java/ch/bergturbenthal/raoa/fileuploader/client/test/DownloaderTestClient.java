...
public class DownloaderTestClient {
    public static void main(final String[] args) throws InterruptedException, ExecutionException, IOException {
        ...
        final RequestFileDownloadServiceBlockingStub requestFileStub = RequestFileDownloadServiceGrpc.newBlockingStub(channel);
        final FileDownloadResponse downloadResponse = requestFileStub.requestFileDownload(Test.FileDownloadRequest.newBuilder().build());

        final File targetFile = File.createTempFile("data", ".out");
        final FileOutputStream outputStream = new FileOutputStream(targetFile);
        final Flux<ByteString> fileData = downloadClient.downloadFile(downloadResponse.getDownloadHandle(), channel);
