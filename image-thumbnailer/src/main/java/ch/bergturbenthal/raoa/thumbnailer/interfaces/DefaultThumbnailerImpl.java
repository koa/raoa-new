...
public class DefaultThumbnailerImpl extends ThumbnailerServiceGrpc.ThumbnailerServiceImplBase {
                        ...
                        () -> {
                          try {
                            ...
                            if (resultCode == 0) {
                              ...
                              logfile.delete();
                              final FileGeneratedMessage fileGeneratedMessage =
                                  FileGeneratedMessage.newBuilder()
                                      .setFileDownloadHandle(fileHandle)
                                      .setFileSize(targetTempFile.length())
                                      .build();
                              responseObserver.onNext(
                                  GenerationResponse.newBuilder()
                                      .setFileReadyResponse(fileGeneratedMessage)
                                      .build());
                            } else {
                              targetTempFile.delete();
                              // TODO: handle error
  ...
  public void getCapabilities(
    ...
    final ThumbnailerCapabilities capabilities =
        ThumbnailerCapabilities.newBuilder().addRegex(".*\\.JPG").build();
