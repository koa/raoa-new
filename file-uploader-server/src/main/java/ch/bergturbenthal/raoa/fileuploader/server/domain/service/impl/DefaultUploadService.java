...
public class DefaultUploadService implements FileuploadService {
  ...
  private final Map<Long, UploadJob> currentPendingJobs =
  ...
  public UploadingFileSlot anounceUpload(final long expectedSize, final String suffix) {
    ...
    try {
      ...
      while (true) {
        ...
        final AtomicReference<ScheduledFuture<?>> pendingScheduledFuture =
            new AtomicReference<ScheduledFuture<?>>(null);
        final Consumer<ScheduledFuture<?>> scheduledFutureConsumer =
            scheduledFuture -> {
              final ScheduledFuture<?> oldFuture =
                  pendingScheduledFuture.getAndSet(scheduledFuture);
        ...
        final Runnable refreshTimeout =
            () -> {
              ...
              scheduledFutureConsumer.accept(
                  executorService.schedule(cleanupRunnable, remainingTime, TimeUnit.MILLISECONDS));
            };
